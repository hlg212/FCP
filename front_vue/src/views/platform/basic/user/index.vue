<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.qco.orgId" placeholder="机构" clearable class="filter-item" style="width: 130px">
    ...
    >
      ...
      <el-table-column label="账号类型" width="100px" align="center">
        <template slot-scope="{row}">
          <el-tag>{{ row.isAdmin | typeFilter }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="机构名称" align="center" width="220" >
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="90px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="账号类型" prop="isAdmin">
          <el-select :disabled='temp.disabled' v-model="temp.isAdmin" class="filter-item" placeholder="Please select">
            <el-option v-for="item in typeOptions" :key="item.value" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="机构名称" >
          <el-input :disabled='temp.disabled' v-model="temp.org.name" placeholder="选择机构"/>
        </el-form-item>
        <el-form-item label="电话" prop="phone">
          <el-input :disabled='temp.disabled' v-model="temp.phone" placeholder="请输入电话"/>
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <el-input :disabled='temp.disabled' v-model="temp.email" placeholder="请输入邮箱地址"/>
        </el-form-item>
...
// const typeOptions = [
//   { key: 'N', display_name: '普通用户' },
//   { key: 'Y', display_name: '超级管理员' }
// ]
...
const typeOptions = DictHelper.getDicts("basic","USER_TYPE");
const typeKeyValue = DictHelper.getDictsMap("basic","USER_TYPE");
